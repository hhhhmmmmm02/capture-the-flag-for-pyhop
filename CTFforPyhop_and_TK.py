...
def CTFtraining(state):
    ...
    for i in range(0,maxTurns):
        action = returnNextAIAction(state,aiPlayer,locDest) ### to be modified by learning algorithm
        ### Here you will call a function that uses information about the action chosen in state
        #print(action)
        state = executeActionAIPlayer(state,aiPlayer,action)
        ### Here you will call a function that uses information about the resulting state and action to learn
        printPlayers(state)
        #input("enter")
        if i%10 == 0:
            print("after ", i, "turns:")
            ...
            textscores = "(Blue "+ str(state.blueScore) + " ; Red " + str(state.redScore) + ")"
            pygame.display.set_caption(textscores)
            #input("enter")
...
def drawGrid(state):
    ...
    for row in range(state.rows):
        ...
        for column in range(state.cols):
            the_grid[row].append(0)  # Append a cell
    ...
    # Set title of screen
    pygame.display.set_caption("(Blue 0; Red 0)")
...
def drawGridAtLoc(state,i,j):
    ...
    textRect = [(MARGIN + WIDTH) * j + MARGIN,
    ...
    #extpos = text.get_rect(centerx=gameWindow.get_width()/2)
    ...
    if nBlueP > 0 and nRedP > 0:
        thetext = font.render(tx, 1, BLACKCOLOR)
    elif nBlueP > 0 and flag != 2:
        thetext = font.render(tx, 1, BLUECOLOR)
    elif nRedP > 0 and flag != 3:
        thetext = font.render(tx, 1, REDCOLOR)
    else: ## nBlueP == 0 and nRedP == 0 or flags are of the same color as player in flag's location
        thetext = font.render(tx, 1, BLACKCOLOR)
    display_surface.blit(thetext, textRect)
...
# This sets the WIDTH and HEIGHT of each grid location in pygame
WIDTH = state.cols
HEIGHT = state.rows
...
MAXHEIGHT = 500
MAXWIDTH = 500
WINDOW_SIZE = [MAXHEIGHT, MAXWIDTH]

# This sets the margin between each cell in pygame
MARGIN = 5
